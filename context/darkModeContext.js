...
export const DarkModeProvider = ({ children }) => {
  ...
  const [lng, setLngState] = useState(() => {
    return isLocalStorageAvailable ? localStorage.getItem("lng") : "ar";
  });
  const setLng = (newLng) => {
    setLngState(newLng);
  ...
  const generateConfetti = () => {
    ...
    if (container) {
      for (let i = 0; i < 100; i++) {
        ...
        confetti.className =
          "confetti " + SHAPES[Math.floor(Math.random() * 60)];
    ...
    const fetchLocation = async () => {
      try {
        ...
        if (storedCountry.country) {
          ...
          const countryObj = countriesWithCities.find(
            (country) => country.country === location.countryName
          );
  ...
  return (
    ...
    >
      {isConfettiActive ? (
        <div
          className="fixed z-50 top-0 left-0 w-full h-full pointer-events-none"
          ref={containerRef}
