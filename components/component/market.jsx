...
export default function Market({ lng, ads, user }) {
  ...
  const { selectedCountry } = useDarkMode();
  ...
  const specificationSelectionProps = {
    ...
    paramsToCheck: [], // Adjust as per your required parameters
  };
  ...
  const conditionSelectorProps = {
    ...
    itemsArray: [t("New"), t("Used")], // Adjust based on your specific translations or labels
    shouldOpen: true, // Adjust as per your logic
  ...
  const MarketAdCard = searchParams.has("country")
    ? dynamic(() => import("./new-card"), {
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {displayedContent === "brands" || !searchParams.has("brand") ? (
              Object.keys(carBrands)
                ...
                .map((brand, i) => (
                  <Button
                    ...
                    className="bg-[#fe26355e] hover:bg-[#fe2635] hover:text-white text-red-800 border border-[#fe2635] dark:text-white"
                  >
                    {brand}
                  </Button>
                ))
              ...
              carBrands[currentBrand].map((model, i) => (
                <Button
                  key={model}
                  onClick={() => handleModleSelect(model)}
                  className="bg-[#fe26355e] hover:bg-[#fe2635] hover:text-white text-red-800 border border-[#fe2635] dark:text-white"
                >
                  {model}
                </Button>
