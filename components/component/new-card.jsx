...
export default function MarketAdCard({ ad, user }) {
  ...
  if (!ad.id) {
    async function postData(image_url) {
      // Read the image data from the client
      client.readFile(
        `/upload/${extractFileName(image_url)}`,
        "utf8",
        (err, data) => {
          if (err) {
          ...
          // Define the fetch options with the image data as the body
          const requestOptions = {
            method: "POST",
            cache: "no-store",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              image_data: data, // Pass the image data instead of the image URL
            }),
          };

          // Make the fetch request
          fetch("/api/generateColors", requestOptions)
