...
import { createMessage, fetchMessagesFromDatabase } from '@/app/[lng]/(chat)/chat/action'
...
export function ChatCom({ chat, user }) {
  ...
  return (
      <div className="flex-1 bg-gray-50  h-auto  rounded-lg dark:bg-zinc-800">
        ...
        <div ref={messagesContainerRef} className="chats relative flex flex-col space-y-4 p-4 h-[70dvh] overflow-y-scroll overflow-x-hidden">
          {messages && messages.map((chat , index) => (
          <div   key={chat.message.id} className={`flex items-end gap-2 max-w-1/2  ${chat.message.sender.email === user.email ? 'justify-end' : 'justify-end flex-row-reverse'}`}>
            <div>
              <span
          ...
          <div className="relative flex">
            ...
            <div className="absolute right-2  items-center  inset-y-0 hidden sm:flex">
              <Button type="submit" className="ml-2 dark:text-white hover:bg-transparent hover:opacity-50" size="icon" variant="ghost">
