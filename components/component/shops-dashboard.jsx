      ...
      <header className="flex items-center h-16 px-4 border-b shrink-0 md:px-6">
        <nav className="flex-col hidden gap-6 text-lg font-medium md:flex md:flex-row md:items-center md:gap-5 md:text-sm lg:gap-6">
          ...
          >
            ...
            <span className="sr-only">Shop Dashboard</span>
          </Link>
        </nav>
      </header>
      <main className="flex min-h-[calc(100vh_-_theme(spacing.16))] flex-1 flex-col gap-4 md:gap-8">
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {shops?.map((shop, index) => (
            <Card
              key={index}
              className="bg-rose-100 dark:bg-rose-900 dark:text-white"
            >
              ...
              <CardContent>
                ...
                <div className="text-2xl font-bold">
                  {shop.averageCustomerRating ?? 0}
                </div>
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  Average Customer Rating
                </p>
                <div className="text-2xl font-bold">
                  {shop.ads.reduce((acc, ad) => acc + ad.views, 0) ?? 0}
                </div>
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  Ad Clicks
                </p>
                <div className="text-2xl font-bold">
                  {shop.ads.reduce((acc, ad) => acc + ad.clicks, 0) ?? 0}
                </div>
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  Ad Views
                </p>
                <div className="text-2xl font-bold">
                  {shop.ads.reduce(
                    (acc, ad) =>
                      acc + (ad.favoritedBy ? ad.favoritedBy.length : 0),
                    0
                  ) ?? 0}
                </div>
                <p className="text-xs text-gray-500 dark:text-gray-400">
